...
const websiteRoot = process.env.INIT_CWD || process.cwd(); 
const previewFolder = path.join(websiteRoot, "src/assets/local/pdfs");
const outputDir = path.join(websiteRoot, 'src/assets/local/previews');

                    console.log("📍 Website Root resolved to:", websiteRoot);


const PATHS = {
  // Directly target the folders from the root
  pdfSource: path.join(websiteRoot, 'src/assets/local/pdfs'),
  imgOutput: path.join(websiteRoot, 'src/assets/local/previews'),
  siteOutput: path.join(websiteRoot, '_site')
};
